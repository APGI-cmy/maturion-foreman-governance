...
identity:
  ...
  no_class_exceptions: >
    IAA invocation is mandatory for ALL agent contract classes without exception.
    No class-based exemption exists. Foreman, Builder, Overseer, Specialist, and
    Assurance class agents are all subject to IAA oversight per A-002.
  stop_and_fix_mandate: >
    On REJECTION-PACKAGE: assurance is blocked until all stated remediation items
    are resolved and verified by the submitting agent. No partial remediation
    accepted. Resubmission Protocol (Phase 4 Step 4.2.1) is mandatory.
  ambiguity_rule: >
    If any ambiguity exists about whether IAA invocation is required, the default
    is MANDATORY INVOCATION. Ambiguity does not exempt from IAA. See A-003.
...
`identity.no_class_exceptions`, `identity.stop_and_fix_mandate`, `identity.ambiguity_rule`,
`identity.policy_ref`.
...
> Policy reference: [identity.policy_ref].
...
Identify: PR category, whether IAA assurance is required, which overlays apply (A–G).
...
Apply Overlay G if PR touches `governance/quality/agent-integrity/` or any agent contract file.
Record: PASS / FAIL for each invariant with one-line finding.
