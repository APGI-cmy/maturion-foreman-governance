...
| `ACTIVE_CS2_AUTOMATED_WORKFLOW_GOVERNANCE.md` | **UPDATED v1.0.1 (2026-09-22) PUBLIC_API** - Defines the contract-ready active-CS2 successor for one authorised parent job with an arbitrary approved ordered wave plan. Includes indexed schemas, fixtures, templates, and control map; requires evaluator-controlled progression, shared safety accounting, independent assurance, scoped machine-governed merge refusal, and integrated whole-job acceptance; activation remains separately human-CS2 approved. | Readiness, Layer-down, PR-gates |
...
| `AGENT_CONTRACT_ARCHITECTURE.md` | **NEW v1.0.0 (2026-02-17) PUBLIC_API** - Canonical four-phase agent contract architecture (Preflight-Induction-Build-Handover) for all Living Agent System v6.2.0 agents. Defines structure, phase-by-phase requirements, benefits, migration path, and enforcement mechanisms. Foundation for standardizing all agent contracts across supervisor, builder, overseer, administrator, and QA classes | PR-gates, Layer-down, Readiness |
...
| `AGENT_CONTRACT_PLACEHOLDER_CHECK_CANON.md` | **NEW v1.0.0 (2026-04-19) LAYER_DOWN** - Canonical exception-class model for agent-contract placeholder-check validation. Defines five bounded exception classes (EXC-001 to EXC-005) for governed meta-language mentions of `stub`, `placeholder`, and `TBD`. Defines non-permitted cases and workflow binding rules. Prevents ad hoc exception-phrase accretion in consumer repo CI. Established under CS2 authority via Issue #1349 | PR-gates, Layer-down, Readiness |
...
| `AGENT_CREATION_BUNDLE_REQUIREMENTS.md` | **NEW v1.0.0 (2026-02-21) PUBLIC_API** - Defines the five mandatory outputs for every new agent creation: (1) contract file, (2) Tier 2 knowledge stubs in `.agent-workspace/<agent>/knowledge/`, (3) `AGENT_REGISTRY.json` entry, (4) routing update, (5) pre-handover commissioning proof. No agent is active until all components verified. Closes gap where partial creation left agents ungovernable. See also `THREE_TIER_AGENT_KNOWLEDGE_ARCHITECTURE.md` and `PROXY_AUTHORITY_MODEL.md` | PR-gates, Layer-down, Readiness |
...
| `FM_QUALITY_PROTOCOL_ENHANCED_SOP.md` | **NEW v1.0.0 (2026-03-02) PUBLIC_API** - Tier-3 governance SOP defining two mandatory enhancements to the Foreman Quality Protocol (QP): (1) formal Builder Referral — every QP FAIL produces a trackable `builder-referral-*.md` artifact and a `REFERRAL_INDEX.md` entry, enabling open-referral visibility and formal closure; (2) Progress Tracker Enforcement — a new QP-FAIL-007 failure code and mandatory gate requiring the progress tracker to be current with the delivered build before the merge gate can be released. Supplements foreman-v2 Quality Professor mode (Mode 3, Section 1.7) without modifying agent contracts. Tier 2 knowledge stub at `.agent-workspace/foreman-v2/knowledge/FM_QP_ENHANCED_QUICK_REFERENCE.md` | PR-gates, Layer-down, Readiness, Feedback/learning |
...
| `GOVERNANCE_TOKEN_USAGE_REQUIREMENTS.md` | **NEW v1.0.0 (2026-03-01) PUBLIC_API** - Canonical policy establishing universal enforcement that all automated write operations MUST use MATURION_BOT_TOKEN. Prohibits github.token and secrets.GITHUB_TOKEN for write operations (commit, push, PR, issue, merge, label, dispatch). Defines 5 requirements (REQ-TU-001 through REQ-TU-005), enforcement via merge gate governance/token-usage-check, consumer repo obligations, and links to motivating isms CI failure. | Layer-down, PR-gates, Readiness |
...
| `GOVERNANCE_LAYER_UP_PROTOCOL.md` | **NEW v1.0.0 (2026-02-21) PUBLIC_API** - Automated layer-up protocol for local governance extensions when consumer repo version exceeds canonical version. Defines local extension detection, canonization candidate PR creation, CS2 approval gate, version guard fields (canonical_version, local_version, local_extension, layer_up_status), integration/rejection flows, and CANON_INVENTORY schema extension. Complements LAYER_UP_PROTOCOL.md (manual layer-up) with automated detection and propagation for consumer version advances. Implements bidirectional governance evolution with full audit trail | Layer-down, Readiness, Feedback/learning |
...
| `ROOT_CAUSE_CORRECTIVE_ACTION_AGENT_CANON.md` | **NEW v1.0.0 (2026-05-08) PUBLIC_API** - Canonical Root Cause and Corrective Action (RCA) model. Defines mandatory/optional/non-trigger conditions, failure-class taxonomy, lowest-effective-layer routing, required RCA assessment output, IAA RCA review (`PASS \| REFER_BACK`), Foreman routing obligations, CS2/agent-advisor overview conditions, phased activation (manual → soft automation → hard gate), and maturion-isms layer-down requirements with anti-burden guardrails. | Feedback/learning, PR-gates, Layer-down |
...
| `PROXY_AUTHORITY_MODEL.md` | **NEW v1.0.0 (2026-02-21) PUBLIC_API** - Canonical model for CS2-to-Foreman proxy authority delegation. Defines required proxy grant language pattern, scope constraints, prohibited operations even under proxy, verification protocol for Foreman/CodexAdvisor/all agents, and proxy lifecycle (granted → active → consumed → expired). Extends `CS2_AGENT_FILE_AUTHORITY_MODEL.md` Level 3. All agents must verify explicit proxy authority before accepting Foreman-created agent files as valid | PR-gates, Layer-down, Readiness |
...
| `THREE_TIER_AGENT_KNOWLEDGE_ARCHITECTURE.md` | **NEW v1.0.0 (2026-02-21) PUBLIC_API** - Canonical 3-tier agent knowledge architecture. Defines Tier 1 (constitutional domain knowledge, immutable, SHA256-verified against CANON_INVENTORY.json), Tier 2 (operational domain knowledge in `.agent-workspace/<agent>/knowledge/`, version-controlled), and Tier 3 (ephemeral session context). Specifies knowledge acquisition protocol, staleness detection and response, inter-tier interaction rules, and per-agent-class application. Required for all new agent creation bundles. References consumer implementation in APGI-cmy/maturion-isms#360, #362 | PR-gates, Layer-down, Readiness |
...
| `ROOT_CAUSE_CORRECTIVE_ACTION_AGENT_STRATEGY.md` | **NEW v1.0.0 (2026-05-08)** - Strategy-first definition of the Root Cause and Corrective Action Agent (RCA Agent / Continuous Improvement Agent) model. Establishes purpose, terminology, trigger philosophy, role boundaries, lowest-effective-layer doctrine, anti-burden guardrails, evidence and handover intent, and phased activation sequence (Strategy → Canon → Activation). | Feedback/learning, Readiness, Layer-down |
